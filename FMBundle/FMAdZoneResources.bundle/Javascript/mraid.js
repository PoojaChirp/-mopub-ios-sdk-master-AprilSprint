// mraid - fractional media

// SDK Messaging handler

(function (window) {
	"use strict";

	function URLMessageHandler(scheme) {
		this.scheme = scheme;
	}

	URLMessageHandler.prototype = {
		send: function (path, queryObject) {
			var iframe = window.document.createElement('iframe');
			iframe.setAttribute('src', this.scheme + "://" + path + "?" + queryObject);
	...
	Object.defineProperty(window, "URLMessageHandler", {
		enumerable: false,
		configurable: false,
		writable: false,
		value: URLMessageHandler
	});
})(this);
// TODO: Overwrite console for logging
// This is currently set up to overwrite the console methods for iOS. In order to ensure we keep the same interface we will need to do this regardless of the context.

(function (window) {
	"use strict";

	var messageHandler = new window.URLMessageHandler('log');

	var _console = console;
	console = {};

	function createLogger(level) {
    	// first parameter is not gonna work out =\
		return function () {
			_console[level].apply( _console, arguments);
	        messageHandler.send(level, "message=" + JSON.stringify(arguments));
		}
	}
...
(function() {
 ...
 console.log("MRAID.JS - executeNativeCall");
